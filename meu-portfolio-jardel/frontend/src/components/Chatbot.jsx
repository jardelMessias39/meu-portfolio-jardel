...
const Chatbot = ({ isOpen, onToggle }) => {
  ...
  const [messages, setMessages] = useState([
    {
      ...
      type: "bot",
      content:
        "Oi, tudo bem? Eu sou o Antônio. Fui criado pelo Jardel para ser o braço direito dele aqui no portfólio. Posso te contar sobre a experiência dele com Full Stack, mostrar os sistemas que ele já tirou do papel ou falar sobre as tecnologias que ele domina. O que você quer saber primeiro?",
      timestamp: new Date(),
    },
  ]);
  ...
  // Função para digitar texto simulando digitação humana
  const dispararDigitação = useCallback((textoCompleto) => {
    let i = 0;
    const novaMensagemId = Date.now();
    setMessages((prev) => [
      ...prev,
      { id: novaMensagemId, type: "bot", content: "", timestamp: new Date() },
    ]);
  ...
  // Função que executa a resposta da IA: digitação + áudio + evento para sincronizar vídeo
  const executarIACompleta = useCallback(
    (texto) => {
      if (!texto) return;

      // 1. Digitar o texto imediatamente
      dispararDigitação(texto);

      // 2. Buscar áudio em paralelo
      fetch(`${API}/tts`, {
        method: "POST",
  ...
  // Função para enviar mensagem para backend
  const handleSendMessage = useCallback(
    async (textoParaEnviar) => {
    ...
    [inputValue, isTyping, sessionId, executarIACompleta, toast],
  );
  ...
  // Sincroniza a ref da função para uso no reconhecimento de voz
  useEffect(() => {
    handleSendRef.current = handleSendMessage;
  }, [handleSendMessage]);

  // Inicializa reconhecimento de voz com tipagem correta e eventos
  useEffect(() => {
  ...
  // Captura erros globais de promessas rejeitadas (ex: áudio)
  useEffect(() => {
    const handleGlobalError = (event) => {
      if (
        event.reason?.message?.includes("ElevenLabs") ||
  ...
  // Renderização condicional para botão de abrir/fechar chatbot
  if (!isOpen)
    return (
      <Button
        onClick={onToggle}
        className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-blue-600 shadow-lg z-50 hover:bg-blue-700 transition-all"
    ...
    <div className="fixed bottom-6 right-6 w-96 h-[500px] bg-white rounded-2xl shadow-2xl border flex flex-col z-50 font-sans animate-in slide-in-from-bottom-5">
      ...
      <div className="bg-blue-600 text-white p-4 rounded-t-2xl flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center ${
              isListening ? "bg-green-400 animate-pulse" : "bg-blue-500"
            }`}
            aria-live="polite"
            aria-atomic="true"
          >
        ...
        <Button
          ...
          variant="ghost"
          size="icon"
          className="text-white h-8 w-8 hover:bg-blue-500"
      ...
      >
        ...
        {isTyping && (
          <div className="text-xs text-gray-400 animate-pulse ml-2">
            Jardel Messias está processando...
          </div>
        )}
